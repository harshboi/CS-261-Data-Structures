...
int bst_size(struct bst* bst) {
  return(get_size(bst->root)+1);
}

int get_size(struct bst_node* node){
  int lheight = 0, rheight = 0;
  if(node->left){
    lheight = get_size(node->left);
    lheight++;
  }
  if(node->right){
    rheight = get_size(node->right);
    rheight++;
  }
...
int bst_height(struct bst* bst) {
  return(check_height(bst->root));
}

int check_height(struct bst_node *node){
  int lheight = 0;
  int rheight = 0;
  if(node->left){
...
int bst_path_sum(int sum, struct bst* bst) {
  // struct bst_node *iter = bst->root;
  int i=0,answer = 0;
  answer = calculate_path(bst->root,sum-bst->root->val);
  // for(int i = 0;i<10;i++){
  //   printf(" i  % f\n",array[i]);
  // }
  // while(i<10){
...
int calculate_path(struct bst_node *node, int sum){
  // int lsum = 0,rsum = 0,i = 0,sum = 0;
  int is_true = 0;
  if(node->left){
    // lsum += calculate_path(node->left,array);
    is_true = calculate_path(node->left,sum - node->left->val);
    if(is_true){
      return 1;
    }
  }
  if(node->right){
    is_true = calculate_path(node->right,sum - node->right->val);
    if(is_true)
      return 1;
    // rsum += calculate_path(node->right,array);
  }
  if(!node->left && !node->right){
    // printf("SUM %f   NODE->VAL%f   \n",sum,node->val);
    if(sum == 0){
      // printf("SUM %f   NODE->VAL%f   \n",sum,node->val);
      return 1;
    }
  ...
  return(is_true);
  return 0;
  // (lsum>0)?(return(lsum)):(return(sum));
  // array[i++] = lsum;
  // array[i++] = rsum;

}

int helper_function(struct bst_node *node,int *array, int sum){
...
int bst_iterator_next(struct bst_iterator* iter) {
  ...
  if(stack_isempty){
    temp->node = stack_pop(iter->stack);
  }
  
  // Value to be returned from the popped item
  int value = temp->node->val;

  // Setting up stack for the next set of elements on the right of the array
  if(temp->node->right){
      temp->node = temp->node->right;
      stack_push(iter->stack,temp->node);
      // Checks for 0
